...
class SGD(keras.optimizers.Optimizer):
    """SGD optimizer.
    from  https://github.com/keras-team/keras/blob/master/keras/optimizers.py#L157
    """
    def __init__(self,
                 lr=0.01,
                 momentum=0.,
...
def pairwise_overlapping_areas(boxes):
    """Computes IoU overlaps between two sets of boxes.
    boxes1, boxes2: [N, (y1, x1, y2, x2)].
    """
    # 1. Tile boxes2 and repeat boxes1. This allows us to compare
    ...
    # using tf.tile() and tf.reshape.
    b1 = tf.reshape(
        tf.tile(tf.expand_dims(boxes, 1), [1, 1, tf.shape(boxes)[0]]), [-1, 4])
    b2 = tf.tile(boxes, [tf.shape(boxes)[0], 1])
    # 2. Compute intersections
    b1_y1, b1_x1, b1_y2, b1_x2 = tf.split(b1, 4, axis=1)
    b2_y1, b2_x1, b2_y2, b2_x2 = tf.split(b2, 4, axis=1)
    y1 = tf.maximum(b1_y1, b2_y1)
    ...
    intersection_area = tf.reshape(
        intersection_area,
        [tf.shape(boxes)[0], tf.shape(boxes)[0]])
    diagonals = tf.zeros(tf.shape(boxes)[0], dtype=tf.float32)
    intersection_area = tf.matrix_set_diag(intersection_area, diagonals)

    return intersection_area


def pairwise_overlapping_x_and_y(boxes):
    """Computes IoU overlaps between two sets of boxes.
    boxes1, boxes2: [N, (y1, x1, y2, x2)].
    """
    # 1. Tile boxes2 and repeat boxes1. This allows us to compare
    ...
    y2 = tf.minimum(b1_y2, b2_y2)
    x2 = tf.minimum(b1_x2, b2_x2)
    intersection_x = tf.maximum(x2 - x1, 0)
    intersection_y = tf.maximum(y2 - y1, 0)
    # 3. Set diagonal elements to zero.
    intersection_x = tf.reshape(
        intersection_x,
        [tf.shape(boxes)[0], tf.shape(boxes)[0]])
    intersection_y = tf.reshape(
        intersection_y,
        [tf.shape(boxes)[0], tf.shape(boxes)[0]])
    diagonals = tf.zeros(tf.shape(boxes)[0], dtype=tf.float32)
    intersection_x = tf.matrix_set_diag(intersection_x, diagonals)
    intersection_y = tf.matrix_set_diag(intersection_y, diagonals)
...
def refine_structure_detections_graph(rois, probs, deltas, window,
                                      gt_class_ids, gt_boxes, gt_start_rows,
                                      gt_start_cols, gt_end_rows, gt_end_cols,
                                      config):
    ...
    roi_gt_boxes = tf.gather(gt_boxes, roi_gt_box_assignment)
    num_detected_boxes = tf.shape(positive_preds)[0]
    ...
    overlapping_areas = pairwise_overlapping_areas(pred_boxes_denorm)
    overlapping_x, overlapping_y = pairwise_overlapping_x_and_y(
        pred_boxes_denorm)
    ...
    gt_boxes_denorm = tf.pad(gt_boxes_denorm, [(0, P), (0, 0)])
    ...
    overlapping_areas = tf.pad(overlapping_areas, [(0, P), (0, P)])
    overlapping_x = tf.pad(overlapping_x, [(0, P), (0, P)])
    overlapping_y = tf.pad(overlapping_y, [(0, P), (0, P)])

    return num_detected_boxes, predicted_flags, pred_boxes, gt_boxes_denorm, pred_boxes_denorm, roi_gt_boxes, stuct_info, overlapping_areas, overlapping_x, overlapping_y
...
class StructureDetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        detections_batch = utils.batch_slice(
            [
                rois, mrcnn_class, mrcnn_bbox, window, gt_class_ids, gt_boxes,
                gt_start_rows, gt_start_cols, gt_end_rows, gt_end_cols
            ], lambda x, y, w, z, a, b,
            c, d, e, f: refine_structure_detections_graph(
                x, y, w, z, a, b, c, d, e, f, self.config),
            self.config.IMAGES_PER_GPU)
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    return detection_flags, detections, pred_boxes_denorm
...
def fpn_classifier_graph(rois,
    ...
    x = PyramidROIAlign([pool_size, pool_size],
                        name="roi_align_classifier")([rois, image_meta] +
                                                     feature_maps)
    x_attention_global_pool = KL.TimeDistributed(
        KL.GlobalAveragePooling2D(), name='mrcnn_attention_global_pool')(x)
    x_attention_reduced_complexity = KL.TimeDistributed(
        KL.Dense(256, kernel_regularizer='l1', activation='relu'),
        name='mrcnn_attention_dense_reduced_complexity')(
            x_attention_global_pool)
    x_attention = KL.TimeDistributed(
        KL.Dense(256, kernel_regularizer='l1', activation='sigmoid'),
        name='mrcnn_attention')(x_attention_reduced_complexity)
    x_attention = KL.Lambda(lambda y: K.expand_dims(y, axis=-2))(x_attention)
    x_attention = KL.Lambda(lambda y: K.expand_dims(y, axis=-2))(x_attention)
    x_attention = KL.Lambda(lambda y: K.tile(y, [1, 1, 7, 7, 1]))(x_attention)
    x = KL.Lambda(lambda y: KL.multiply(y))([x, x_attention])
    ...
    loss_weight_logits = KL.TimeDistributed(
        KL.Dense(num_loss_components, use_bias=False),
        name='mrcnn_loss_weights_logits')(shared)
    loss_weights = KL.TimeDistributed(
        KL.Activation("softmax"),
        name="mrcnn_loss_weights")(loss_weight_logits)
    ...
    return mrcnn_class_logits, mrcnn_probs, mrcnn_bbox, loss_weights
...
def build_fpn_mask_graph(rois,
    ...
    x_attention_global_pool = KL.TimeDistributed(
        ...
        name='mrcnn_mask_attention_global_pool')(x)
    x_attention_reduced_complexity = KL.TimeDistributed(
        KL.Dense(256, kernel_regularizer='l1', activation='relu'),
        name='mrcnn_mask_attention_dense_reduced_complexity')(
            x_attention_global_pool)
    x_attention = KL.TimeDistributed(
        KL.Dense(256, kernel_regularizer='l1', activation='sigmoid'),
        name='mrcnn_mask_attention')(x_attention_reduced_complexity)
    x_attention = KL.Lambda(lambda y: K.expand_dims(y, axis=-2))(x_attention)
    x_attention = KL.Lambda(lambda y: K.expand_dims(y, axis=-2))(x_attention)
    x_attention = KL.Lambda(lambda y: K.tile(y, [1, 1, 14, 14, 1]))(
        x_attention)
    x = KL.Lambda(lambda y: KL.multiply(y))([x, x_attention])
    ...
    x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2),
                                              strides=2,
                                              activation="relu"),
                           name="mrcnn_mask_deconv_1")(x)
    """x_attention_conv2d = KL.TimeDistributed(KL.Conv2D(256, (1, 1),
                                                      padding="same"),
                                            name='mrcnn_attention_conv2d_2')(x)
    x_attention_reduced_complexity = KL.TimeDistributed(
        KL.Dense(64, kernel_regularizer='l1', activation='relu'),
        name='mrcnn_mask_attention_dense_reduced_complexity_2')(
            x_attention_conv2d)
    x_attention = KL.TimeDistributed(
        KL.Dense(256, kernel_regularizer='l1', activation='sigmoid'),
        name='mrcnn_mask_attention_2')(x_attention_reduced_complexity)
    x = KL.Lambda(lambda y: KL.multiply(y))([x, x_attention])
    x = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2),
                                              strides=2,
                                              activation="relu"),
                           name="mrcnn_mask_deconv_2")(x)"""
...
def rpn_bbox_structural_loss_graph(roi_box_coordinate, roi_gt_coordinate,
                                   segments, num_detected_boxes, loss_weights):
    ...
    mask_pairwise = tf.tile(tf.expand_dims(mask_pairwise, -1), [1, 1, 1, 4])
    ...
    loss = tf.compat.v1.losses.mean_squared_error(
        roi_gt_box_pairwise_diff, roi_box_pairwise_diff,
        reduction='none') * mask_pairwise
    # loss = tf.cast(tf.reduce_sum(loss, [1, 2, 3]),
    #                tf.float32) / (tf.cast(num_detected_boxes, tf.float32) +
    #                               tf.constant(0.00000001, dtype=tf.float32))
    loss = tf.cast(tf.reduce_sum(loss, [2, 3]), tf.float32) * loss_weights
    #loss = tf.reduce_mean(loss)
    loss = tf.cast(tf.math.reduce_sum(loss, 1), tf.float32) / (
        tf.cast(tf.math.count_nonzero(loss), tf.float32) +
        tf.constant(0.000001, tf.float32))
    loss = tf.reduce_mean(loss)
    return loss


def bbox_overlapping_loss_graph(overlapping_region, num_detected_boxes,
                                loss_weights):
    ...
    mask_pairwise = tf.expand_dims(mask, 1) * tf.expand_dims(mask, 2)
    loss = tf.compat.v1.losses.mean_squared_error(
        tf.zeros_like(overlapping_region),
        overlapping_region,
        reduction='none') * mask_pairwise
    # loss = tf.cast(tf.reduce_sum(loss, [1, 2]),
    #                tf.float32) / (tf.cast(num_detected_boxes, tf.float32) +
    #                               tf.constant(0.00000001, dtype=tf.float32))
    loss = tf.cast(tf.reduce_sum(loss, [2]), tf.float32) * loss_weights
    #loss = tf.reduce_mean(loss)
    loss = tf.cast(tf.math.reduce_sum(loss, 1), tf.float32) / (
        tf.cast(tf.math.count_nonzero(loss), tf.float32) +
        tf.constant(0.000001, tf.float32))
...
def rpn_bbox_structural_continuity_loss_graph(roi_box_coordinate,
                                              roi_gt_coordinate, segments,
                                              num_detected_boxes,
                                              loss_weights):
    ...
    start_rows, start_cols, end_rows, end_cols = tf.split(segments, 4, axis=-1)
    start_rows = tf.cast(tf.squeeze(start_rows, axis=-1), tf.int32)
    start_cols = tf.cast(tf.squeeze(start_cols, axis=-1), tf.int32)
    end_rows = tf.cast(tf.squeeze(end_rows, axis=-1), tf.int32)
    end_cols = tf.cast(tf.squeeze(end_cols, axis=-1), tf.int32)

    gy1, gx1, gy2, gx2 = tf.split(roi_gt_coordinate, 4, axis=-1)
    gy1 = tf.cast(tf.squeeze(gy1, axis=-1), tf.int32)
    gx1 = tf.cast(tf.squeeze(gx1, axis=-1), tf.int32)
    gy2 = tf.cast(tf.squeeze(gy2, axis=-1), tf.int32)
    gx2 = tf.cast(tf.squeeze(gx2, axis=-1), tf.int32)

    y1, x1, y2, x2 = tf.split(roi_box_coordinate, 4, axis=-1)
    y1 = tf.cast(tf.squeeze(y1, axis=-1), tf.int32)
    x1 = tf.cast(tf.squeeze(x1, axis=-1), tf.int32)
    y2 = tf.cast(tf.squeeze(y2, axis=-1), tf.int32)
    x2 = tf.cast(tf.squeeze(x2, axis=-1), tf.int32)

    row_continuity_segments = tf.equal(
        tf.expand_dims(start_rows, 1),
        tf.expand_dims(end_rows + tf.constant(1, tf.int32), 2))
    row_continuity_segments = tf.cast(row_continuity_segments,
                                      tf.float32) * mask_pairwise
    col_continuity_segments = tf.equal(
        tf.expand_dims(start_cols, 1),
        tf.expand_dims(end_cols + tf.constant(1, tf.int32), 2))
    col_continuity_segments = tf.cast(col_continuity_segments,
                                      tf.float32) * mask_pairwise
    gt_pairwise_start_end_x_differences = tf.cast(
        tf.expand_dims(gx1, 1) - tf.expand_dims(gx2, 2), tf.float32)
    pairwise_start_end_x_differences = tf.cast(
        tf.expand_dims(x1, 1) - tf.expand_dims(x2, 2), tf.float32)
    gt_pairwise_start_end_y_differences = tf.cast(
        tf.expand_dims(gy1, 1) - tf.expand_dims(gy2, 2), tf.float32)
    pairwise_start_end_y_differences = tf.cast(
        tf.expand_dims(y1, 1) - tf.expand_dims(y2, 2), tf.float32)
    gt_pairwise_start_end_x_differences = gt_pairwise_start_end_x_differences * col_continuity_segments * mask_pairwise
    pairwise_start_end_x_differences = pairwise_start_end_x_differences * col_continuity_segments * mask_pairwise
    gt_pairwise_start_end_y_differences = gt_pairwise_start_end_y_differences * row_continuity_segments * mask_pairwise
    pairwise_start_end_y_differences = pairwise_start_end_y_differences * row_continuity_segments * mask_pairwise
    loss_row_continuity = tf.compat.v1.losses.mean_squared_error(
        gt_pairwise_start_end_y_differences,
        pairwise_start_end_y_differences,
        reduction='none') * mask_pairwise
    loss_col_continuity = tf.compat.v1.losses.mean_squared_error(
        gt_pairwise_start_end_x_differences,
        pairwise_start_end_x_differences,
        reduction='none') * mask_pairwise
    loss = tf.cast(tf.add(loss_row_continuity, loss_col_continuity),
                   tf.float32)
    # loss = tf.cast(tf.reduce_sum(loss, [1, 2]),
    #                tf.float32) / (tf.cast(num_detected_boxes, tf.float32) +
    #                               tf.constant(0.00000001, dtype=tf.float32))
...
def compute_average_overlapping_area(num_detected_boxes, overlapping_areas):
    mask = tf.sequence_mask(num_detected_boxes, ROIS)
    mask = tf.cast(mask, tf.float32)
    mask_pairwise = tf.expand_dims(mask, 1) * tf.expand_dims(mask, 2)
    mask_pairwise = tf.cast(mask_pairwise, tf.float32)
    overlapping_areas = overlapping_areas * mask_pairwise
    overlapping_areas_avg = tf.cast(
        tf.math.reduce_sum(overlapping_areas), tf.float32) / (
            tf.cast(tf.math.count_nonzero(overlapping_areas), tf.float32) +
            tf.constant(0.000001, tf.float32))
    overlapping_areas_avg = overlapping_areas_avg / tf.constant(
        2.0, tf.float32)
    return overlapping_areas_avg


def compute_weights_average(num_detected_boxes, loss_weights):
    mask = tf.sequence_mask(num_detected_boxes, ROIS)
    mask = tf.cast(mask, tf.float32)
    loss_weights = loss_weights * mask
    weights_avg = tf.cast(tf.math.reduce_sum(loss_weights), tf.float32) / (
        tf.cast(tf.math.count_nonzero(loss_weights), tf.float32) +
        tf.constant(0.000001, tf.float32))
    return weights_avg


def compute_weights_std(num_detected_boxes, loss_weights):
    mask = tf.sequence_mask(num_detected_boxes, ROIS)
    mask = tf.cast(mask, tf.float32)
    loss_weights = loss_weights * mask
    weights_avg = tf.cast(tf.math.reduce_sum(loss_weights), tf.float32) / (
        tf.cast(tf.math.count_nonzero(loss_weights), tf.float32) +
        tf.constant(0.000001, tf.float32))
    weights_dev = tf.math.subtract(loss_weights, weights_avg) * mask
    weights_std = tf.cast(tf.math.reduce_sum(weights_dev), tf.float32) / (
        tf.cast(tf.math.count_nonzero(weights_dev), tf.float32) +
        tf.constant(0.000001, tf.float32))
    return weights_std
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            global ROIS
            ROIS = config.TRAIN_ROIS_PER_IMAGE
            ...
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox, loss_weights =\
            ...
            # loss_weights = KL.Lambda(lambda x: tf.where(
            #     x < tf.constant(0.01,
            #                     shape=(self.config.BATCH_SIZE, self.config.
            #                            TRAIN_ROIS_PER_IMAGE, 3)),
            #     tf.constant(0.99,
            #                 shape=(self.config.BATCH_SIZE, self.config.
            #                        TRAIN_ROIS_PER_IMAGE, 3)), tf.identity(x)))(
            #                            loss_weights)
            # loss_weights = KL.Lambda(lambda x: tf.where(
            #     x > tf.constant(0.99,
            #                     shape=(self.config.BATCH_SIZE, self.config.
            ...
            # loss_weights = KL.TimeDistributed(
            #     KL.Activation("softmax"),
            #     name="mrcnn_loss_weights_rescale")(loss_weights)
            alignment_loss_weights = KL.Lambda(lambda x: x[:, :, 0])(
                loss_weights)
            continuity_loss_weights = KL.Lambda(lambda x: x[:, :, 1])(
                loss_weights)
            overlap_x_loss_weights = KL.Lambda(lambda x: x[:, :, 2])(
                loss_weights)
            overlap_y_loss_weights = KL.Lambda(lambda x: x[:, :, 3])(
                loss_weights)

            num_detected_boxes, predicted_flags, detection_boxes, gt_boxes_denorm, detection_boxes_denorm, roi_gt_boxes, stuct_info, overlapping_areas, overlapping_x, overlapping_y = StructureDetectionLayer(
            ...
            avg_overlapping_area = KL.Lambda(
                lambda x: compute_average_overlapping_area(*x),
                name="avg_overlapping_area")(
                    [num_detected_boxes, overlapping_areas])
            ...
            roi_alignment_loss = KL.Lambda(
                ...
                name="roi_alignment_loss")([
                    detection_boxes_denorm, gt_boxes_denorm, stuct_info,
                    num_detected_boxes, alignment_loss_weights
                ])
            roi_continuity_alignment_loss = KL.Lambda(
                lambda x: rpn_bbox_structural_continuity_loss_graph(*x),
                name="roi_continuity_alignment_loss")([
                    detection_boxes_denorm, gt_boxes_denorm, stuct_info,
                    num_detected_boxes, continuity_loss_weights
                ])

            roi_overlapping_x_loss = KL.Lambda(
                lambda x: bbox_overlapping_loss_graph(*x),
                name="roi_overlapping_x_loss")([
                    overlapping_x, num_detected_boxes, overlap_x_loss_weights
                ])
            roi_overlapping_y_loss = KL.Lambda(
                lambda x: bbox_overlapping_loss_graph(*x),
                name="roi_overlapping_y_loss")([
                    overlapping_y, num_detected_boxes, overlap_y_loss_weights
                ])
            avg_alignment_weight = KL.Lambda(
                lambda x: compute_weights_average(*x),
                name="avg_alignment_weight")(
                    [num_detected_boxes, alignment_loss_weights])

            avg_continuity_weight = KL.Lambda(
                lambda x: compute_weights_average(*x),
                name="avg_continuity_weight")(
                    [num_detected_boxes, continuity_loss_weights])

            avg_overlap_x_weight = KL.Lambda(
                lambda x: compute_weights_average(*x),
                name="avg_overlap_x_weight")(
                    [num_detected_boxes, overlap_x_loss_weights])

            avg_overlap_y_weight = KL.Lambda(
                lambda x: compute_weights_average(*x),
                name="avg_overlap_y_weight")(
                    [num_detected_boxes, overlap_y_loss_weights])

            std_alignment_weight = KL.Lambda(lambda x: compute_weights_std(*x),
                                             name="std_alignment_weight")([
                                                 num_detected_boxes,
                                                 alignment_loss_weights
                                             ])

            std_continuity_weight = KL.Lambda(
                lambda x: compute_weights_std(*x),
                name="std_continuity_weight")(
                    [num_detected_boxes, continuity_loss_weights])

            std_overlap_x_weight = KL.Lambda(lambda x: compute_weights_std(*x),
                                             name="std_overlap_x_weight")([
                                                 num_detected_boxes,
                                                 overlap_x_loss_weights
                                             ])

            std_overlap_y_weight = KL.Lambda(lambda x: compute_weights_std(*x),
                                             name="std_overlap_y_weight")([
                                                 num_detected_boxes,
                                                 overlap_y_loss_weights
                                             ])
            ...
            outputs = [
                ...
                roi_alignment_loss, roi_continuity_alignment_loss,
                roi_overlapping_x_loss, roi_overlapping_y_loss,
                avg_alignment_weight, std_alignment_weight,
                avg_continuity_weight, std_continuity_weight,
                avg_overlap_x_weight, std_overlap_x_weight,
                avg_overlap_y_weight, std_overlap_y_weight,
                avg_overlapping_area
            ]
            ...
            # normalized coordinates
            detection_flags, detections, pred_boxes_denorm = DetectionLayer(
                config, name="mrcnn_detection")(
            ...
            model = KM.Model([input_image, input_image_meta, input_anchors], [
                ...
                rpn_class, rpn_bbox, loss_weights
            ],
                             name='mask_rcnn')
    ...
    def compile(self, learning_rate, momentum):
        ...
        loss_names = [
            ...
            "roi_continuity_alignment_loss", "roi_overlapping_x_loss",
            "roi_overlapping_y_loss"
        ]
        ...
        metric_names = [
            ...
            "mrcnn_bbox_loss", "mrcnn_mask_loss", "roi_alignment_loss",
            "roi_continuity_alignment_loss", "roi_overlapping_x_loss",
            "roi_overlapping_y_loss", "avg_alignment_weight",
            "std_alignment_weight", "avg_continuity_weight",
            "std_continuity_weight", "avg_overlap_x_weight",
            "std_overlap_x_weight", "avg_overlap_y_weight",
            "std_overlap_y_weight", "avg_overlapping_area"
        ]
    ...
    def train(self,
        ...
        layer_regex = {
            ...
            "heads":
            r"(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)|(td\_.*)|(bu\_.*)|(gate\_.*)|(graph\_.*)",
            # From a specific Resnet stage and up
            "3+":
            r"(res3.*)|(bn3.*)|(res4.*)|(bn4.*)|(res5.*)|(bn5.*)|(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)|(td\_.*)|(bu\_.*)|(gate\_.*)|(graph\_.*)",
            "4+":
            r"(res4.*)|(bn4.*)|(res5.*)|(bn5.*)|(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)|(td\_.*)|(bu\_.*)|(gate\_.*)|(graph\_.*)",
            "5+":
            r"(res5.*)|(bn5.*)|(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)|(td\_.*)|(bu\_.*)|(gate\_.*)|(graph\_.*)",
            # All layers
            "all": ".*",
        }
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
                          image_shape, window, loss_weights):
        ...
        return boxes, class_ids, scores, full_masks, weights
    ...
    def detect(self, images, verbose=0):
        ...
        detections, _, _, mrcnn_mask, _, _, _, loss_weights =\
            self.keras_model.predict([molded_images, image_metas, anchors], verbose=0)
        # Process detections
        results = []
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks , final_loss_weights=\
                self.unmold_detections(detections[i], mrcnn_mask[i],
                                       image.shape, molded_images[i].shape,
                                       windows[i], loss_weights[i])
            results.append({
                "rois": final_rois,
                "class_ids": final_class_ids,
                "scores": final_scores,
                "masks": final_masks,
                "weights": final_loss_weights,
            })
        return results
